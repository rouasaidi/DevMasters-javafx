...
public class Signin {
    ...
    void singuser(ActionEvent event) {
        ...
        try {
            usercrud crud = new usercrud(); // Instancier usercrud
            if (crud.existeEmail(email)) {
                // Afficher une alerte indiquant que l'adresse e-mail existe déjà
                Alert errorAlert = new Alert(Alert.AlertType.ERROR);
                errorAlert.setTitle("Error");
                errorAlert.setHeaderText(null);
                errorAlert.setContentText("This email address is already registered.");
                errorAlert.showAndWait();
                return; // Arrêter l'exécution de la méthode si l'adresse e-mail existe déjà
            }
            ...
            errorAlert.setTitle("Error");
            errorAlert.setHeaderText(null);
            errorAlert.setContentText("Failed to check email existence. SQLException: " + e.getMessage());
            errorAlert.showAndWait();
            return; // Arrêter l'exécution de la méthode en cas d'erreur SQL
        }
        ...
        try {
            ...
            envoyeur.envoyer(email, usernametextfiled.getText(), phoneNumber);
